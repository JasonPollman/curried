...
describe('combine', () => {
  ...
  it('Should shallow combine 1 object', () => {
    const a = { a: 1 };
    expect(combine(a)).toEqual({ a: 1 });
    expect(combine(a)).not.toBe(a);
  });

  it('Should handle bad input', () => {
    const a = { a: 1 };
    expect(combine(a, null, 4, undefined)).toEqual({ a: 1 });
  });

  it('Should shallow combine 2 objects', () => {
    const a = { a: 1 };
    expect(combine(a, { b: 2 })).toEqual({ a: 1, b: 2 });
    expect(combine(a, { b: 2 })).not.toBe(a);
  });
  ...
  it('Should shallow combine 3+ objects', () => {
    ...
    const combined = combine(x1, { a: 1 }, x2, { b: 2 });
  ...
  it('Should shallow combine multiple types of objects', () => {
    ...
    const combined = combine(x1, { a: 1 }, x2, { b: 2 }, map, set, arr, str);
