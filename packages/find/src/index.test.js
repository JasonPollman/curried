...
describe('find', () => {
  ...
  describe('find.f', () => {
    it('Should be a function', () => {
      ...
      expect(typeof find.f).toBe('function');
      expect(typeof f).toBe('function');
    });

    it('Should find within an array', () => {
      expect(f(isTwo)([1, 2, 3, 4])).toEqual(2);
    });

    it('Should work with shorthand string iteratees', () => {
      expect(f('x')([{ x: true }, { x: false }])).toEqual({ x: true });
      expect(f('x', [{ x: true }, { x: false }])).toEqual({ x: true });
    });

    it('Should work with shorthand array iteratees', () => {
      expect(f(['value', 2], [{ value: 1 }, { value: 2 }, { value: 3 }])).toEqual({ value: 2 });
      expect(f(['value', 2])([{ value: 1 }, { value: 2 }, { value: 3 }])).toEqual({ value: 2 });
    });
    ...
    it('Should work with shorthand object iteratees', () => {
      ...
      expect(f({ value: 2, name: 'c' })(things)).toEqual({ value: 2, name: 'c', foo: 'bar' });
    });
